...
namespace cmudb {
  ...
  bool TablePage::InsertTuple(const Tuple& tuple, RID& rid, Transaction* txn,
    ...
    if (ENABLE_LOGGING) {
      ...
      LogRecord insertRecord(txn->GetTransactionId(), txn->GetPrevLSN(), LogRecordType::INSERT, rid, tuple);
      log_manager->AppendLogRecord(insertRecord);
      txn->SetPrevLSN(insertRecord.GetLSN());
      SetLSN(insertRecord.GetLSN());
    }
  ...
  bool TablePage::MarkDelete(const RID& rid, Transaction* txn,
    ...
    if (ENABLE_LOGGING) {
      ...
      Tuple tuple;
      LogRecord markDeleteRecord(txn->GetTransactionId(), txn->GetPrevLSN(), LogRecordType::MARKDELETE, rid, tuple);
      log_manager->AppendLogRecord(markDeleteRecord);
      txn->SetPrevLSN(markDeleteRecord.GetLSN());
      SetLSN(markDeleteRecord.GetLSN());
    }
  ...
  bool TablePage::UpdateTuple(const Tuple& new_tuple, Tuple& old_tuple,
    ...
    if (ENABLE_LOGGING) {
      ...
      LogRecord updateRecord(txn->GetTransactionId(), txn->GetPrevLSN(), LogRecordType::UPDATE, rid, old_tuple, new_tuple);
      log_manager->AppendLogRecord(updateRecord);
      txn->SetPrevLSN(updateRecord.GetLSN());
      SetLSN(updateRecord.GetLSN());
    }
  ...
  void TablePage::ApplyDelete(const RID& rid, Transaction* txn,
    ...
    if (ENABLE_LOGGING) {
      ...
      assert(txn->GetExclusiveLockSet()->find(rid) !=
        txn->GetExclusiveLockSet()->end());
      LogRecord deleteRecord(txn->GetTransactionId(), txn->GetPrevLSN(), LogRecordType::APPLYDELETE, rid, delete_tuple);
      log_manager->AppendLogRecord(deleteRecord);
      txn->SetPrevLSN(deleteRecord.GetLSN());
      SetLSN(deleteRecord.GetLSN());
    }
  ...
  void TablePage::RollbackDelete(const RID& rid, Transaction* txn,
    ...
    if (ENABLE_LOGGING) {
      ...
      assert(txn->GetExclusiveLockSet()->find(rid) !=
        txn->GetExclusiveLockSet()->end());
      Tuple tuple;
      LogRecord deleteRecord(txn->GetTransactionId(), txn->GetPrevLSN(), LogRecordType::ROLLBACKDELETE, rid, tuple);
      log_manager->AppendLogRecord(deleteRecord);
      txn->SetPrevLSN(deleteRecord.GetLSN());
      SetLSN(deleteRecord.GetLSN());
    }
